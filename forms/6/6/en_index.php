...
if ($_POST && isset($_POST['name']) && isset($_POST['comment'])) {
    ...
    if (empty($name) || empty($comment)) {
        $_SESSION['error'] = "Name and comment are required";
    } elseif (!validateLength($name, 50) || !validateLength($comment, 1000) || !validateLength($recommendation, 100)) {
        $_SESSION['error'] = "The character limit has been exceeded";
    } elseif (!canPost()) {
        $_SESSION['error'] = "You have reached the posting limit (up to 5 times per hour).";
    } elseif (hasNGWords($comment) || hasNGWords($name) || hasNGWords($recommendation)) {
        $_SESSION['error'] = "It contains inappropriate language";
    } else {
